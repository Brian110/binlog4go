...
func (res *Result) MakeRow() Row {
	return make(Row, res.field_count)
}

func (my *MySQL) getResult(res *Result, row Row) *Result {
loop:
...
func (my *MySQL) getTextRowPacket(pr *pktReader, res *Result, row Row) {
	if my.Debug {
...
func (my *MySQL) getBinRowPacket(pr *pktReader, res *Result, row Row) {
	if my.Debug {
