...
func (pr *pktReader) readDate() Date {
	y, m, d := pr.readTime().Date()
	return Date{int16(y), byte(m), byte(d)}
}

func EncodeDate(buf []byte, d Date) int {
	if d.IsZero() {
		// MySQL zero
		buf[0] = 0
		return 1
...
func (pw *pktWriter) writeDate(d Date) {
	buf := pw.buf[:5]
	n := EncodeDate(buf, d)
	pw.write(buf[:n])
}

func lenDate(d Date) int {
	if d.IsZero() {
		return 1
	}
	return 5
